...
document.getElementById('register-form').addEventListener('submit', function(event) {
    ...
    if (userType === 'doctor') {
        ...
        var workSchedule = collectWorkIntervals(); // Collect the work schedule intervals
        if (!workSchedule || workSchedule.length === 0) {
            alert('Please fill in the work schedule for the doctor.');
            return; // Exit the function to prevent form submission
        }

        formData.work_schedule = workSchedule;
    }
    fetch('/ligonine/admin/doctorRegistry/processRegister.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            window.location.href ='/ligonine/index.php'; // Redirect to login after registration
        } else {
